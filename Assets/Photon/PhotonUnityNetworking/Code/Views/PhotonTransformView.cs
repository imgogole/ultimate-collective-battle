...
namespace Photon.Pun
{
    ...
    public class PhotonTransformView : MonoBehaviourPun, IPunObservable
    {
        ...
        public float m_MinDistanceTeleporting = 10f;
        ...
        public void Update()
        {
            ...
            if (!this.photonView.IsMine)
            {
                if (m_UseLocal)
                {
                    if (m_Distance > m_MinDistanceTeleporting)
                    {
                        tr.SetLocalPositionAndRotation(this.m_NetworkPosition, this.m_NetworkRotation);
                    }
                    else
                    {
                        tr.SetLocalPositionAndRotation(Vector3.MoveTowards(tr.localPosition, this.m_NetworkPosition, this.m_Distance * Time.deltaTime * PhotonNetwork.SerializationRate), Quaternion.RotateTowards(tr.localRotation, this.m_NetworkRotation, this.m_Angle * Time.deltaTime * PhotonNetwork.SerializationRate));
                    }
                }
                else
                {
                    if (m_Distance > m_MinDistanceTeleporting)
                    {
                        tr.SetPositionAndRotation(this.m_NetworkPosition, this.m_NetworkRotation);
                    }
                    else
                    {
                        tr.SetPositionAndRotation(Vector3.MoveTowards(tr.position, this.m_NetworkPosition, this.m_Distance * Time.deltaTime * PhotonNetwork.SerializationRate), Quaternion.RotateTowards(tr.rotation, this.m_NetworkRotation, this.m_Angle * Time.deltaTime * PhotonNetwork.SerializationRate));
                    }
                }
            }
        }
